...
function mkStep(from: number, to: number, val: string | null) {
  if (val == '+em') return new AddMarkStep(from, to, schema.marks.em.create())
  else if (val == '-em') return new RemoveMarkStep(from, to, schema.marks.em.create())
  else
    return new ReplaceStep(
      from,
      to,
      val == null ? Slice.empty : new Slice(Fragment.from(schema.text(val)), 0, 0)
    )
}
...
describe('Step', () => {
  describe('merge', () => {
    function yes(
      ...
      return () => {
        let step1 = mkStep(from1, to1, val1),
